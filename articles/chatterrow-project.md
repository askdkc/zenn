...
「でもクラウドだとデータや記録を全部ベンダーに握られちゃうな〜」

「オンプレで運用出来ないかな〜」

### 『それ、chatterrowなら一つにまとめられます』

という感じのオンプレ向けWEBサービスです👍
...
### オンプレなのでチャットも資料も自分たちのデータ

chatterrowをオンプレで運用すると、チャットのやり取りを含む全てのデータを自分たちのデータとして利活用できます。

アップロードしたOfficeファイルとPDFは自動的にMarkdownへ変換され、下記のディレクトリに保存されます。

```text
APP_DIR/storage/markdowned-docs
```

日々のチャットには、業務の判断に至った経緯や、マニュアルには書かれていない各種ノウハウ、社内の暗黙知がどんどん蓄積されていきます。

さらに、OfficeファイルやPDFもMarkdownとして保存されるため、文章データとして非常に扱いやすくなります。

これらのデータをまとめて活用することで、社内の各種ノウハウや暗黙知を、自社で利用する生成AIのトレーニングに使いやすい形で蓄積できます🤖

チャットも社内資料も自分たちのサーバーに置いたまま、生成AIで活用しやすいデータに変えられるというわけです👍
