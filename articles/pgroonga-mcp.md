...
最近のAIモデルはSQLを書くのが得意です。やりたいことを伝えれば、テーブル作成や検索SQLなどをすぐに作ってくれます。インデックスを使った高速化も、かなり正確に提案してくれます。ところが、日本語検索になると話が変わります。表記揺れや漢字の異体字などを考え始めると、検索を期待どおりに動かすまでの調整が急に難しくなります。

日本語の全文検索に便利なPGroongaというPostgreSQL用の拡張機能があります。検索で使い勝手の良い中間一致にインデックスを利かすことができ、日本語検索も非常に高速です。ただ、インデックス作成やノーマライザー、トークナイザーの使い方に癖があり、条件次第では思ったように動作しないことも度々ありました。AIもこの辺は苦手だったため、毎回あれこれ修正のやり取りが頻発するのも嫌になり、PGroonga用のMCPサーバーを作りました。
